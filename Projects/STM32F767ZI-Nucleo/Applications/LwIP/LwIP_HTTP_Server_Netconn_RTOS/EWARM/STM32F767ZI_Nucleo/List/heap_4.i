...
/* Allocate the memory for the heap. */
	static uint8_t ucHeap[ ( ( size_t ) ( 20 * 1024 ) ) ];
...
void *pvReturn = 0;
		...
		if( ( xWantedSize & xBlockAllocatedBit ) == 0 )
		{
			...
			if( xWantedSize > 0 )
			{
				...
				if( ( xWantedSize & ( 0x0007 ) ) != 0x00 )
				{
					...
					if( ( ( xWantedSize & ( 0x0007 ) ) == 0 ) == 0 ) { { __iar_builtin_disable_interrupt(); __iar_builtin_set_BASEPRI( ( 5 << (8 - 4) ) ); __iar_builtin_DSBx(15); __iar_builtin_ISBx(15); __iar_builtin_enable_interrupt(); }; for( ;; ); };
				}
				else
				{
					;
			...
			if( ( xWantedSize > 0 ) && ( xWantedSize <= xFreeBytesRemaining ) )
			{
				...
				if( pxBlock != pxEnd )
				{
					...
					if( ( pxBlock->xBlockSize - xWantedSize ) > ( ( size_t ) ( xHeapStructSize << 1 ) ) )
					{
						...
						if( ( ( ( ( size_t ) pxNewBlockLink ) & ( 0x0007 ) ) == 0 ) == 0 ) { { __iar_builtin_disable_interrupt(); __iar_builtin_set_BASEPRI( ( 5 << (8 - 4) ) ); __iar_builtin_DSBx(15); __iar_builtin_ISBx(15); __iar_builtin_enable_interrupt(); }; for( ;; ); };
	...
	if( ( ( ( ( size_t ) pvReturn ) & ( size_t ) ( 0x0007 ) ) == 0 ) == 0 ) { { __iar_builtin_disable_interrupt(); __iar_builtin_set_BASEPRI( ( 5 << (8 - 4) ) ); __iar_builtin_DSBx(15); __iar_builtin_ISBx(15); __iar_builtin_enable_interrupt(); }; for( ;; ); };
	return pvReturn;
}
/*-----------------------------------------------------------*/
...
BlockLink_t *pxLink;
	...
	if( pv != 0 )
	{
		...
		if( ( ( pxLink->xBlockSize & xBlockAllocatedBit ) != 0 ) == 0 ) { { __iar_builtin_disable_interrupt(); __iar_builtin_set_BASEPRI( ( 5 << (8 - 4) ) ); __iar_builtin_DSBx(15); __iar_builtin_ISBx(15); __iar_builtin_enable_interrupt(); }; for( ;; ); };
		if( ( pxLink->pxNextFreeBlock == 0 ) == 0 ) { { __iar_builtin_disable_interrupt(); __iar_builtin_set_BASEPRI( ( 5 << (8 - 4) ) ); __iar_builtin_DSBx(15); __iar_builtin_ISBx(15); __iar_builtin_enable_interrupt(); }; for( ;; ); };
...
size_t uxAddress;
size_t xTotalHeapSize = ( ( size_t ) ( 20 * 1024 ) );
